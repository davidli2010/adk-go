...
- 内置工具集:
  ...
  - `LoadMemoryTool`: 加载长期记忆工具
  - `PreloadMemoryTool`: 预加载记忆工具
...
**实现支持**:
- `inmemory/`: 内存会话存储（开发和测试）
- `database/`: SQLite持久化存储（生产环境）
  - 支持会话历史持久化
  - 状态数据持久化
  - 事件流持久化
- `vertexai/`: Vertex AI会话服务
...
- `llminternal/`: LLM相关内部实现
  - `googlellm/`: Google LLM内部实现
  - `converters/`: 类型转换工具
  - 状态管理、回调类型定义、工具调用内部逻辑
- `memory/`: 内存管理内部实现
- `sessioninternal/`: 会话管理内部实现
- `sessionutils/`: 会话工具函数
- `artifact/`: 工件存储内部实现
- `plugininternal/`: 插件系统内部实现
  - `plugincontext/`: 插件上下文管理
  - 插件管理器、生命周期管理
- `toolinternal/`: 工具内部实现
  - `toolutils/`: 工具函数
- `context/`: 上下文实现
...
- `launcher/`: 统一的启动器框架
  ...
  - `web/`: Web启动器
    ...
    - API端点
    - A2A协议
  - `internal/telemetry/`: 启动器遥测集成
- `adkgo/`: ADK Go命令行工具
  - `internal/root/`: 根命令
  - `internal/deploy/`: 部署工具
    - `cloudrun/`: Google Cloud Run部署
...
- `loggingplugin/`: 日志记录插件
  - 事件日志记录
  - 调试信息输出
- `retryandreflect/`: 重试和反射插件
  - 失败重试机制
  - 执行结果反思
- `functioncallmodifier/`: 函数调用修改器
  - 动态修改工具调用参数
  - 结果后处理
